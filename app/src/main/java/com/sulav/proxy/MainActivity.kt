...
class MainActivity:Activity(){
 ...
 override fun onBackPressed(){
  // Keep navigation inside the app instead of unexpectedly leaving a detail/sub-screen.
  if(currentScreen!="CAPTURE") { showCapture(); return }
  val now=System.currentTimeMillis()
  if(now-lastBackAt < 1800L){ super.onBackPressed() }
  else { lastBackAt=now; toast("Press back again to exit") }
 }
 ...
 private fun showCapture(){currentScreen="CAPTURE";reset("CAPTURE CONSOLE",if(proxyRunning) "RUNNING • 0.0.0.0:$proxyPort • HTTPS tunnel ready" else "IDLE • background proxy service");val c=card();label("FORWARD TARGET");val e=edit("https://clientbp.ggpolarbear.com/",target);c.addView(e);c.addView(button("SAVE TARGET"){target=e.text.toString().trim();getPreferences(0).edit().putString("target",target).apply();toast("Target saved")});label("LOCAL CAPTURE");val row=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL};val start=button(if(proxyRunning)"STOP PROXY" else "START PROXY"){if(proxyRunning)stopProxy() else startProxy()};row.addView(start,LinearLayout.LayoutParams(0,dp(54),1f));val clear=button("CLEAR LOG"){CaptureStore.clear(filesDir);captures.clear();updatePacketCount();showCapture()};row.addView(clear,LinearLayout.LayoutParams(0,dp(54),1f));c.addView(row);c.addView(button("VIEW CAPTURED REQUESTS"){showCaptureLog()});content.addView(c);addText(if(captures.isEmpty())"NO PACKETS YET\n\nStart the local proxy or use Request to test an endpoint you control." else "${captures.size} captured request(s)",14f,MUTED,false,20,22)}
 ...
 private fun startProxy(){
  ...
  if (android.os.Build.VERSION.SDK_INT >= 26) startForegroundService(i) else startService(i); proxyRunning=true; updatePacketCount(); showCapture()
 }
